...
is_macos=false
work_dir="/etc/xray"
shortcut_path="/usr/bin/2go"

if [ "$os_name" = "Darwin" ]; then
    is_macos=true
    work_dir="/usr/local/etc/xray"
    shortcut_path="/usr/local/bin/2go"
fi
...
random_port() {
    if command -v shuf &>/dev/null; then
        shuf -i 1000-60000 -n 1
    elif command -v jot &>/dev/null; then
        jot -r 1 1000 60000
    else
        awk 'BEGIN { srand(); print int(1000 + rand() * 59001) }'
    fi
}
...
# 定义环境变量
export UUID=${UUID:-$(uuidgen_value)}
export PORT=${PORT:-$(random_port)}
...
if [ -f "${work_dir}/${server_name}" ]; then
    if [ "$is_macos" = true ]; then
        pgrep -f "${work_dir}/${server_name} run -c ${config_dir}" >/dev/null 2>&1 && green "running" && return 0 || yellow "not running" && return 1
...
if [ -f "${work_dir}/argo" ]; then
    if [ "$is_macos" = true ]; then
        pgrep -f "${work_dir}/argo tunnel --url http://localhost:${ARGO_PORT}" >/dev/null 2>&1 && green "running" && return 0 || yellow "not running" && return 1
    elif [ -f /etc/alpine-release ]; then
...
macos_only_install_notice() {
    yellow "\nmacOS 当前支持安装、配置生成和临时 Argo 域名。\n"
    yellow "不支持 Caddy、systemd/openrc 托管、固定隧道管理等 Linux 专属功能。\n"
    green "手动启动命令：${work_dir}/${server_name} run -c ${config_dir}\n"
}

is_macos_unsupported_flow() {
    if [ "$is_macos" = true ]; then
        yellow "macOS 当前仅支持安装、查看节点信息和临时 Argo 域名，此菜单暂不支持。"
        sleep 1
        menu
        return 0
    fi
...
parse_argo_domain() {
    local argo_log_file="${work_dir}/argo.log"

    [ ! -f "$argo_log_file" ] && return 1
...
install_xray() {
    ...
    if [ "$is_macos" = true ]; then
        case "${ARCH_RAW}" in
            'x86_64') XRAY_ASSET="Xray-macos-64.zip"; CLOUDflared_ASSET="cloudflared-darwin-amd64.tgz" ;;
            'aarch64' | 'arm64') XRAY_ASSET="Xray-macos-arm64-v8a.zip"; CLOUDflared_ASSET="cloudflared-darwin-arm64.tgz" ;;
            *) red "macOS 暂不支持该架构: ${ARCH_RAW}"; exit 1 ;;
        esac
    else
        XRAY_ASSET="Xray-linux-${ARCH_ARG}.zip"
        CLOUDflared_ASSET="cloudflared-linux-${ARCH}"
    fi
    ...
    rm -rf "${work_dir}/${server_name}.zip" "${work_dir}/argo.tgz" "${work_dir}/geosite.dat" "${work_dir}/geoip.dat" "${work_dir}/README.md" "${work_dir}/LICENSE"
   ...
   # 生成随机UUID和密码
    ...
    output=$(${work_dir}/${server_name} x25519)
...
get_info() {
  ...
  if [ -f "${work_dir}/argo.log" ] || [ "$is_macos" = true ]; then
...
vmess://$(echo "{ \"v\": \"2\", \"ps\": \"${isp}\", \"add\": \"${CFIP}\", \"port\": \"${CFPORT}\", \"id\": \"${UUID}\", \"aid\": \"0\", \"scy\": \"none\", \"net\": \"ws\", \"type\": \"none\", \"host\": \"${argodomain}\", \"path\": \"/vmess-argo?ed=2560\", \"tls\": \"tls\", \"sni\": \"${argodomain}\", \"alpn\": \"\", \"fp\": \"chrome\"}" | base64_nowrap)
EOF
      if [ "$is_macos" != true ]; then
          base64_nowrap ${work_dir}/url.txt > ${work_dir}/sub.txt
      fi
  fi
  ...
  if [ "$is_macos" = true ]; then
      macos_only_install_notice
      green "配置文件：${config_dir}\n"
      green "节点信息：${client_dir}\n"
  else
...
if [ ${check_argo} -eq 0 ]; then
    yellow "\n正在停止 Argo 服务\n"
    if [ "$is_macos" = true ]; then
        pkill -f "${work_dir}/argo tunnel --url http://localhost:${ARGO_PORT}" >/dev/null 2>&1
    elif [ -f /etc/alpine-release ]; then
...
if [ ${check_argo} -eq 0 ]; then
    ...
    if [ "$is_macos" = true ]; then
        pkill -f "${work_dir}/argo tunnel --url http://localhost:${ARGO_PORT}" >/dev/null 2>&1 || true
        start_argo_process_macos
    elif [ -f /etc/alpine-release ]; then
...
uninstall_xray() {
   ...
   case "${choice}" in
       y|Y)
           yellow "正在卸载 xray"
           if [ "$is_macos" = true ]; then
                pkill -f "${work_dir}/${server_name} run -c ${config_dir}" >/dev/null 2>&1 || true
           elif [ -f /etc/alpine-release ]; then
                rc-service xray stop
...
EOF
  ...
  ln -sf "$work_dir/2go.sh" "$shortcut_path"
...
# 变更配置
change_config() {
if is_macos_unsupported_flow; then
    return
fi
...
case "${choice}" in
    1)
        reading "\n请输入新的UUID: " new_uuid
        [ -z "$new_uuid" ] && new_uuid=$(uuidgen_value) && green "\n生成的UUID为：$new_uuid"
        ...
        if [ "$is_macos" != true ]; then
            base64_nowrap $client_dir > ${work_dir}/sub.txt
        fi
...
if [ -f "${work_dir}/argo.log" ]; then
  for i in {1..5}; do
      get_argodomain=$(parse_argo_domain)
...
change_argo_domain() {
    if [ "$is_macos" != true ]; then
        sed -i "5s/sni=[^&]*/sni=$ArgoDomain/; 5s/host=[^&]*/host=$ArgoDomain/" "$client_dir"
    fi
    content=$(cat "$client_dir")
    vmess_urls=$(grep -o 'vmess://[^ ]*' "$client_dir")
    vmess_prefix="vmess://"
...
while true; do
   ...
   case "${choice}" in
        ...
        8)
            if [ "$is_macos" = true ]; then
                yellow "macOS 当前未适配该附加功能"
            else
                clear && curl -fsSL https://raw.githubusercontent.com/eooce/ssh_tool/main/ssh_tool.sh -o ssh_tool.sh && chmod +x ssh_tool.sh && ./ssh_tool.sh
            fi
            ;;
        9)
            if [ "$is_macos" = true ]; then
                ...
                clear && bash <(curl -Ls https://raw.githubusercontent.com/eooce/sing-box/main/sing-box.sh)
            fi
            ;;
        0) exit 0 ;;
        *) red "无效的选项，请输入 0 到 9" ;;
